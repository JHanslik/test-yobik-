export function loadColliders(world: CANNON.World): void {
    ...
    const goalShape = new CANNON.Box(new CANNON.Vec3(2.5, 1.9, 0.2));
    ...
    wallWest.quaternion.setFromAxisAngle(
    ...
    wallEast.quaternion.setFromAxisAngle(
        new CANNON.Vec3(0, 1, 0),
        -Math.PI / 2
    );
    world.addBody(wallEast);

    const goal = new CANNON.Body({
        mass: 0,
        shape: goalShape,
        position: new CANNON.Vec3(2, 0.5, 15),
    });
    goal.quaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), -Math.PI / 2);
